...
include '../libs/func.php'; 
    ...
    if(!empty($_POST)) {
        ...
        $isOK = true;   //confirm to insert data into database or not
        $errors = array();
        ...
        if($isOK) {
            //save to database
            $state = insertPost($post_title, $post_desc, $post_thumb, $post_content, $category_id);
            unset($post_title);
            unset($post_desc);
            unset($post_thumb);
            unset($post_content);
    ...
    <body>
        <form style="width: 50%; margin: auto;" class="vertical" method="post" action="">
            <fieldset>
                ...
                <div class="notice success <?php if($state) {echo 'show';} else {echo 'hide';} ?>">Thêm mới bài viết thành công !!!</div>
